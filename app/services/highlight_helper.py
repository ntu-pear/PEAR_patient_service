...
from app.logger.logger_utils import ActionType, log_crud_action
...
def _highlight_snapshot(highlight: PatientHighlight) -> dict:
    """Build a plain dict snapshot of a PatientHighlight row for audit logging."""
    return {
        "Id": highlight.Id,
...
def create_highlight_if_needed(
    ...
    user_full_name is optional: callers that don't pass it (e.g. patient_problem_crud.py,
    patient_vital_router.py) fall back to created_by for the logged full name.
    """
    effective_user_full_name = user_full_name if user_full_name is not None else created_by
    try:
        ...
        if not highlight_type.IsEnabled:
            ...
            if existing_highlight:
                ...
                original_data = _highlight_snapshot(existing_highlight)
                ...
                log_crud_action(
                    ...
                    user=created_by,
                    user_full_name=effective_user_full_name,
                    message=f"Deleted highlight {existing_highlight.Id} because type '{type_code}' is disabled",
                    table="PatientHighlight",
                    entity_id=existing_highlight.Id,
        ...
        if should_highlight and not existing_highlight:
            ...
            log_crud_action(
                ...
                user=created_by,
                user_full_name=effective_user_full_name,
                message=f"Created highlight for {source_table}:{source_record_id}: {highlight_text}",
                table="PatientHighlight",
                entity_id=new_highlight.Id,
                original_data=None,
                updated_data=_highlight_snapshot(new_highlight),
                patient_id=patient_id,
                patient_full_name=None,
                log_type="highlight",
            )
        ...
        # Case 2: Should highlight and existing highlight -> UPDATE
            ...
            log_crud_action(
                ...
                user=created_by,
                user_full_name=effective_user_full_name,
                message=f"Updated highlight {existing_highlight.Id} text from '{old_text}' to '{new_text}'",
                table="PatientHighlight",
                entity_id=existing_highlight.Id,
                original_data={"HighlightText": old_text},
                updated_data={"HighlightText": new_text},
                patient_id=patient_id,
                patient_full_name=None,
                log_type="highlight",
            )
        ...
        # Case 3: Should NOT highlight but existing highlight -> DELETE
            ...
            log_crud_action(
                ...
                user=created_by,
                user_full_name=effective_user_full_name,
                message=f"Deleted highlight {existing_highlight.Id} for {source_table}:{source_record_id} (no longer qualifies)",
                table="PatientHighlight",
                entity_id=existing_highlight.Id,
                original_data=original_data,
                updated_data=None,
