...
def get_patient_guardian_by_nric(nric: str, db: Session = Depends(get_db)):
    ...
    db_guardian_rels = crud_patient_patient_guardian.get_all_patient_patient_guardian_by_guardianNRIC(db, nric)
    if not db_guardian_rels:
        raise HTTPException(status_code=404, detail="No patient relationships found for this guardian")
    return db_guardian_rels
...
def get_patient_guardian_by_patient_id(patient_id: int, db: Session = Depends(get_db)):
    return crud_patient_patient_guardian.get_all_patient_guardian_by_patientId(db, patient_id)
...
def delete_patient_guardian(guardian_id: int, db: Session = Depends(get_db)):
    ...
    db_relationship = crud_patient_patient_guardian.delete_patient_patient_guardian_by_guardianId(db, guardian_id)
    if not db_relationship:
        raise HTTPException(status_code=404, detail="No relationship found")
    
    mapping = crud_relationship.get_relationship_mapping(db, db_relationship.relationshipId)
    relationship_name = mapping.relationshipName if mapping else "Unknown"
    ...
    return PatientGuardianUpdate(
        ...
        ModifiedById=db_guardian.ModifiedById,
        patientId=db_relationship.patientId,
