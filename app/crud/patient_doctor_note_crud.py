...
def get_doctor_note(db: Session, patient_id: int):
...
def update_doctor_note(db: Session, doctor_note: PatientDoctorNoteUpdate):
    db_doctor_note = db.query(PatientDoctorNote).filter(PatientDoctorNote.id == doctor_note.patientId).first()
    if db_doctor_note:
